...
def read_from_memory(include_double=False):
	...
	net_output_single = np.array(tmp).T.reshape((single_inclusion_num,4096)) # converting to a NumPy array
	tmp = f['Y_save'].value
	...
	if include_double :
		...
		net_output_double = np.array(tmp).T.reshape((double_inclusion_num,4096)) # For converting to a NumPy array
		tmp = f1['Y_save'].value
...
def read_preprocess_data(subtract_homogen=True, is_standard_scalar_input=True , noise_level=0.0):
	"""
	...
	X_train, x_test, Y_train, y_test, _, id_test = train_test_split( net_input ,  \
                                                                    net_output, \
                                                                    indices, \
                                                                   test_size=test_num, \
                                                                   random_state=4)
...
def min_fit(net_input):
	net_input_ = (net_input - np.min(net_input,axis=1).reshape(net_input.shape[0],1)) + 0.1
	return net_input_
...
def one_fit(net_input):
	...
	"""
	net_input_ = (net_input + ( 1 - np.max(net_input,axis=1).reshape(net_input.shape[0],1)))
	return net_input_
...
def add_noise(net_input, level=0.0):
	net_input_ = net_input + np.random.normal(0.0, 1, net_input.shape) * level * net_input
	return net_input_
